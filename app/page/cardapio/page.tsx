...
export default function cardapio(){
    ...
    return(
        <div className="p-[20px]">
            ...
            {isloading ? <SkeletonSelecionarProduto></SkeletonSelecionarProduto>: 
                <div className="mt-[10px] overflow-y-auto">
                    ...
                    <div className="flex w-full mt-3 overflow-x-scroll container">
                        ...
                        {categorias.map((item, index) => (
                            <span key={index} className={`inline-block text-[#000] me-3 ${pathname == item.url ? `bg-[${configStore.category.bgActivate}]` : `border-1 border-[${configStore.category.border}] bg-[${configStore.category.bgNotActivate}]`} ps-5 pe-5 pt-2 pb-2 rounded-full`}>{item.titulo}</span>
                        ))}
                    </div>
                    <div className="mt-6">
